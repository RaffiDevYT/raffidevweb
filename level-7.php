<?php 
session_start();
if (!isset($_SESSION["level-6.php"])) {
    header("Location: level-6.php");
    exit;
}
?>
<!DOCTYPE html>
...
if ( isset($_POST["submit"]) ) {
   ...
   if ( $_POST["username"] == $username && $_POST["password"] == $password ) {
    $_SESSION["level-7.php"] = true;
    header("Location: level-8.php");
    exit;
    echo "<script>
    Swal.fire({ title: `Good`,
                text: `Your username and password are correct!`, 
                icon: `success`,
                footer: `<a href='level-8' style='color: #0c1220; background-color: aqua;' class='btn btn-primary'>Continue</a>`,
                 });
